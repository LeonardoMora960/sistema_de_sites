    ...
    <div class="contenido formulario" style="min-height:400px;margin-top:152px">
		...
		?>
			...
			<div class="w380 fl ml_20">
				...
				<select name="<?php echo $row['campo']?>" id="<?php echo $row['campo']?>" class="bordercaja w200 fl lh_15 mb_7" 
					...
					<?php foreach($tabla as $row_tabla): ?>
						<option value="<?php echo $row_tabla->id ?>"><?php echo $row_tabla->vNombre ?></option>
					<?php endforeach ?>
			...
			<div class="w380 fr ml_20">
				...
				<label class="label w140 lh_20 mb_2"><?php echo $row['vNombre']?>:</label>
				<?php $type = ($row['campo'] == 'Contrasena')? "password" : "text"; ?>
				<input id="<?php echo $row['campo']?>" name="<?php echo $row['campo']?>"   type="<?=$type?>" class="bordercaja w200 fl lh_15 mb_7 <?php if($row['iIdTipoCampo'] == 6)echo "calendario"?>" 
		  ...
		  <?php echo form_close();?>
			...
			<div class="fr w250">
				...
				<?php endif;?>
				<input type="button" name="" value="Cancelar" class="btn fl ml_10" onclick="dialogo_zebra_confirmacion('¿desea cancelar el Registro de Usuario y perder los datos ingresados?','question','Alerta de Registro de Usuario',412,'<?php echo base_url().'usuario/registro_usuario/busqueda_usuario'?>')">
			</div>
